...
EC_KEY* GeneratePublicDeterministicKey(const std::vector<unsigned char>& generator, int seq);
EC_KEY* GeneratePrivateDeterministicKey (RippleAddress const& pubGen, const BIGNUM* rootPrivKey, int seq);
static BIGNUM* makeHash(const std::vector<unsigned char>& generator, int seq, BIGNUM* order);
...
public:
	...
	CKey(const std::vector<unsigned char>& generator, int n) : fSet(false)
	...
	void GetPrivateKeyU (uint256& privKey)
	{
		...
		if (bn == nullptr)
			throw std::runtime_error("CKey::GetPrivateKeyU: EC_KEY_get0_private_key failed");
...
static BIGNUM* makeHash(const std::vector<unsigned char>& generator, int seq, BIGNUM* order)
{
    ...
    do
    {
		std::vector<unsigned char> s = generator;
